...
def print_output_graph(input_array,timestep=2e-9,downsample_val=10,L=1,use_filter=True,a=13):
    ...
    for i in range(len(input_array)):
        ...
        if i%downsample_val ==0:
            ...
            processed[i]=(reconstructor.update(input_array[i],ma1,ma2,use_filter))/a
...
def plot_wave(data, sampling_rate=None):
    """
    Plot a 1-D array of samples as a waveform.

    Parameters
    ----------
    data : list or 1D numpy array
        The sequence of amplitude samples to plot.
    sampling_rate : float, optional
        If provided (in Hz), the x-axis will be in seconds; otherwise
        the x-axis is simply the sample index.
    """
    # Build x axis: time if sampling_rate given, otherwise index
    if sampling_rate is not None:
        t = [i / sampling_rate for i in range(len(data))]
        plt.plot(t, data)
        plt.xlabel("Time (s)")
    else:
        plt.plot(data)
...
#100hz about 3k samples/s, 200hz about 4.5k
